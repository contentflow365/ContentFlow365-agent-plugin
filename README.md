...
- Cursor: accepts this Agent Plugin format. Public marketplace submission requires a public Git repository and Cursor review. Cursor's [plugin help](https://prod.cursor.com/help/customization/plugins) describes listed plugins as open source. This package currently grants no open-source license, so the publisher must resolve licensing before submission. The application repository stays private.
